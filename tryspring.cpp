...
struct body
{
	vect r, v, a;
	//body *p1, *p2;
	void show (vect a1 , vect a2, vect a3, int i, int j)
	{
	//	s.draw_circle_solid(r.x, r.y, 2, 0, 0, 200, 0);
		int x[4]={r.x, a2.x, a3.x, a1.x};
		int y[4]={r.y, a2.y, a3.y, a1.y};
	//	s.polygon_solid(x, y, 4, 1, 1.5*(N-j), 0, 1.5*j);
		s.polygon_solid(x, y, 4, 1, data[3*(j*40+i)], data[3*(j*40+i)+1], data[3*(j*40+i)+2]);	
	}
	...
	while (1)
	{
		...
		for (int k=0; k<20; k++)
		{
			for (int i=1; i<N-1; i++)
			{
				for (int j=1; j<N-1; j++)
				{
					if (!(i==selecti && j==selectj && click==1))
			...
			for (int i=1; i<N-1; i++)
			{
				spring[0][i].update_a(0, spring[0][i-1].r, spring[0][i+1].r, spring[1][i].r, spring[0][i].r);
			}
		...
		//	spring[N-1][N-1].update_a(N-1, spring[N-1][N-2].r, spring[N-2][N-1].r, spring[N-1][N-1].r, spring[N-1][N-1].r);
			...
			for (int i=0; i<N; i++)
			{
				for (int j=0; j<N; j++)
				{
					if (!(i==N/2 && j==0)&&!(i==selecti && j==selectj && click==1))
		...
		for (int i=0; i<N-1; i++)
		{
			...
			{
			//	spring[i][j].update_pos();
				spring[i][j].show(spring[i+1][j].r, spring[i][j+1].r, spring[i+1][j+1].r, j);
			}
		}
		/*
		for (int i=0; i<N-1; i++)
