...
Screen s(w, w);
space_3d cam(100, 0.3, 1/2, 1, 100, 100);
...
struct body
{
	...
	void show (vect a1 , vect a2, vect a3)
	{
	//	s.draw_circle_solid(r.x, r.y, 2, 0, 0, 200, 0);
		int x[4]={r2d.x, a2.x, a3.x, a1.x};
		int y[4]={r2d.y, a2.y, a3.y, a1.y};
		s.polygon_solid(x, y, 4, 1, 0, 10, 0);
	}
...
int main()
{
	...
	while (1)
	{
		...
		for (int i=0; i<N-1; i++)
		{
			...
			{
			//	spring[i][j].update_pos();
				spring[i][j].show(spring[i+1][j].r, spring[i][j+1].r, spring[i+1][j+1].r);
			}
		}
		/*
		for (int i=0; i<N-1; i++)
