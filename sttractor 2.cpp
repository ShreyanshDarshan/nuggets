...
*/	
	void checkp(vect r2)
	{
		...
		if (mod(dir)>2)
		v = add(mult(K*sin(mod(dir)/50)/sin(atan(1.0)), dir), v);
	}
	...
	*/
	...
	//	if (!(time%1000))
		for (int i=0; i<N; i++)
		{
			if (part[i].state == 1)
			{
				part[i].r.x = rand()%size1;
				part[i].r.y = rand()%size2;
				part[i].state = 0;
				part[i].color = rand()%5+1;
				part[i].v = {float(rand()%100 - 50)/100, float(rand()%100 - 50)/100};
			}
		}
		...
		for (int i=0; i<size1; i++)
		{
			for (int j=0; j<size2; j++)
			{
				...
				p.put_pixel(i, j, r_,g_,b_);//255-r_, 255-g_, 255-b_);
...
/*		if (time%100)
		for (int i=0; i<size1; i++)
		{
			for (int j=0; j<size2; j++)
			{
				...
				s.get_pixel(i, j, r_, g_, b_);
				if (r_>10)
				p.put_pixel(i, j, r_-10,g_-10,b_-10);//255-r_, 255-g_, 255-b_);
				//s.put_pixel_2(i, j, 50, 50, 50);
			}
		}
*/		
